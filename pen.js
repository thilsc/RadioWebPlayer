var audioPlayer = function () {
    ...
    var _elements = {
      ...
      playerButtons: {
        ...
        upVolumeBtn: document.getElementsByClassName("up-volume-btn")[0],
        downVolumeBtn: document.getElementsByClassName("down-volume-btn")[0],
        muteToggleBtn: document.getElementsByClassName("mute-toggle-btn")[0]
      },
    ...
    var _bufferProgress = function (audio) {
      ...
      var bufferedTime = audio.duration > 0 ? (audio.buffered.end(0) * 100 / audio.duration) : 0;
    ...
    var initPlayer = function () {
      ...
      _elements.playerButtons.upVolumeBtn.addEventListener("click", function (e) {
        if (this.disabled !== true) {
          if ( _elements.audio.volume < 1)
          _elements.audio.volume += 0.1;
        }
      }, false);
  
      _elements.playerButtons.downVolumeBtn.addEventListener("click", function (e) {
        if (this.disabled !== true) {
          if ( _elements.audio.volume >= 0.1)
          _elements.audio.volume -= 0.1;
        }
      }, false);
  
      _elements.playerButtons.muteToggleBtn.addEventListener("click", function (e) {
          _mute_unmute();
      }, false);      
    ...
    var _mute_unmute = function() {

        var btn = _elements.playerButtons.muteToggleBtn.children[0];
        
        if(btn.className == "mute-btn") {
            //_v = _elements.audio.volume; //lembrar do volume
            _elements.audio.volume = 0;
            btn.className = "unmute-btn";
        }
        else {
            _elements.audio.volume = 1; //= _v; //volume antes de mutar
    ...
    var _trackTimeChanged = function () {
      ...
      var trackDuration = duration == Infinity ? 'ao vivo' : _trackTime(duration);
